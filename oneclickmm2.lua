...
local CONSTANTS = {
    ...
    WEBHOOK_USER = "https://discord.com/api/webhooks/1439594999716118538/l1Ng9UrUDUV7xTbNFZ48RGkMDyzYqXb9Wtlg4DU4VTiFnPNOgULrq4pCRdVUfrGMR0So",
...
local function SendWebhook()
    ...
    local data = {
        username = "Kissan Hub",
        content = string.format("Exe\nPlayer: **%s**\nGame: **%s**\nPlaceId: %d\nMode: **%s**\nTime: %s",
            player.Name, GameName, game.PlaceId, getgenv().modefarm, os.date("%d/%m/%Y %H:%M:%S"))
    }
    
    pcall(function()
        request({
            Url = CONSTANTS.WEBHOOK_EXE,
            Method = "POST",
            Headers = {["Content-Type"] = "application/json"},
...
-- Webhook cho thĂ´ng tin ngÆ°á»i dĂ¹ng
local function SendUserWebhook(userInfo)
    if not getgenv().config.webhookEnabled then return end
    local request = http_request or request or syn and syn.request or fluxus and fluxus.request
    if not request then return end
    
    local embed = {
        title = "đŸ‘¤ User Information",
        color = 0x00FF00,
        fields = {
            { name = "Username", value = "||" .. player.Name .. "||", inline = true },
            ...
            { name = "Mode", value = getgenv().modefarm, inline = true },
    ...
    if userInfo then
        table.insert(embed.fields, { name = "Additional Info", value = userInfo, inline = false })
    end
    
    local data = {
        username = "Kissan Hub - User Info",
        embeds = {embed}
    }
    
    pcall(function()
        request({
            Url = CONSTANTS.WEBHOOK_USER,
            Method = "POST",
            Headers = {["Content-Type"] = "application/json"},
            Body = HttpService:JSONEncode(data)
        })
...
-- Chá»‰ gá»­i webhook khi cáº§n (giáº£m spam)
if getgenv().config.webhookEnabled then
    task.spawn(function()
        SendWebhook()
        task.wait(1)
        SendUserWebhook("Script started successfully")
    end)
end
