...
local cachedPaths = {}
function WaitForChildPath(parent, path)
    local key = tostring(parent) .. table.concat(path, ".")
    if cachedPaths[key] then
...
task.spawn(function()
    ...
    for _, fx in pairs(Lighting:GetChildren()) do  
        if fx:IsA("PostEffect") or fx:IsA("Atmosphere") or fx:IsA("Sky") or fx:IsA("Clouds") then  
            fx.Enabled = false  
        end  
    end  
...
function fadeInUI()
    Frame.Visible = true
    Blur.Enabled = true
    TweenService:Create(Blur, TweenInfo.new(0.5, Enum.EasingStyle.Quad), {Size = 35}):Play()
...
function fadeOutUI()
    TweenService:Create(Blur, TweenInfo.new(0.4, Enum.EasingStyle.Quad), {Size = 0}):Play()
    task.wait(0.4)
    Blur.Enabled = false
    Frame.Visible = false
    for _, label in ipairs({Title, CandiesLabel, TierLabel, TimeLabel}) do
...
task.spawn(function()
    while true do
        if Frame.Visible then
            ...
            if not success and CandiesLabel.Text ~= "Total Candies: Error" then  
...
local tierTextLabel = nil
local lastTierText = nil
local tierRetries = 0
local maxTierRetries = 3

task.spawn(function()
    local paths = {
        {"CrossPlatform", "CurrentEventFrame", "Container", "EventFrames", "BattlePass", "Info", "YourTier", "TextLabel"},
        {"MainGui", "BattlePass", "TierLabel"},
        {"BattlePass", "CurrentTier"}
    }

    for _, path in ipairs(paths) do  
        tierTextLabel = WaitForChildPath(player.PlayerGui, path)  
        if tierTextLabel then break end  
    end  
      
    while true do  
        if Frame.Visible and tierTextLabel then  
            local success = pcall(function()  
                local text = tierTextLabel.Text  
                if text ~= lastTierText then  
                    local current, max = string.match(text, "(%d+)%s*/%s*(%d+)")  
                    if current and max then  
                        TierLabel.Text = "Tier: " .. current .. " / " .. max  
                    else  
                        TierLabel.Text = "Tier: " .. text  
                    end  
                    lastTierText = text  
                    tierRetries = 0  
                end  
            ...
            if not success then  
                tierRetries += 1  
                if tierRetries >= maxTierRetries and TierLabel.Text ~= "Tier: Error" then  
...
task.spawn(function()
    while true do
        ...
        if Frame.Visible then  
            local newTime = hours .. "h:" .. minutes .. "m:" .. seconds .. "s"  
            if newTime ~= lastTimeUpdate then  
                TimeLabel.Text = "Client Time Elapsed: " .. newTime  
