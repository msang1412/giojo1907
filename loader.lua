...
local function setupAutoOpenBox()
    ...
    local function openBox(resource)
        if ProfileData and ProfileData.Materials and ProfileData.Materials.Owned then
            ...
            -- KIỂM TRA NẾU SỐ CANDIES >= 800
            if resource == currency then
                local currentCandies = ProfileData.Materials.Owned.Candies2025 or 0
                if currentCandies < 800 then
                    print("Not enough candies! Need 800, have: " .. currentCandies)
                    return false
                end
            end
    ...
    local function startAutoOpen()
        while getgenv().config.autoOpenBox and task.wait(3) do
            -- Kiểm tra số candies trước khi mở hộp
            local currentCandies = ProfileData.Materials.Owned.Candies2025 or 0
            if currentCandies >= 800 then
                ...
                print("Waiting for 800+ candies. Current: " .. currentCandies)
                task.wait(10) -- Chờ lâu hơn nếu không đủ candies
            end
        end
    end
